    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || activeView === 'mypes'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex md:hidden items-center gap-2">
          <button
            onClick={onOpenSearch}
            className={`p-2 rounded-full ${(scrolled || activeView === 'mypes') ? 'bg-slate-100 text-slate-700' : 'bg-white/10 text-white'}`}
          >
            <Search className="w-5 h-5" />
          </button>
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className={`p-2 rounded-lg ${(scrolled || activeView === 'mypes') ? 'text-slate-800' : 'text-white'}`}
          >
